...
export default function DashboardLayout() {
  ...
  return (
    <div className="min-h-screen bg-transparent flex flex-col md:flex-row relative">
      {/* Mobile Header */}
      <div className="md:hidden bg-white dark:bg-dark-surface border-b border-slate-100 dark:border-dark-border p-4 flex items-center justify-between sticky top-0 z-40">
        <div className="flex items-center gap-2">
          <div className="bg-violet-600 p-2 rounded-xl">
            <GraduationCap className="w-5 h-5 text-white" />
          </div>
          <span className="font-extrabold text-lg text-slate-800 dark:text-dark-text-main tracking-tight">TestYourself</span>
        </div>
        <button 
          onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
          className="p-2 text-slate-600 dark:text-dark-text-muted hover:bg-slate-50 dark:hover:bg-dark-bg rounded-lg"
        >
